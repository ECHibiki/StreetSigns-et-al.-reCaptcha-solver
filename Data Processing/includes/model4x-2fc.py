...
def cnnModel(training_data, channels, classes, dimension_x, dimension_y):
    ...
    # fourth convolutional layer -- maps 64 feature maps to 256.
    with tf.name_scope('conv2'):
        W_conv4 = weight_variable([5, 5, 64, 128*4])
        b_conv4 = bias_variable([128*4])
        h_conv4 = tf.nn.relu(conv2d(c3_drop, W_conv4) + b_conv4)

    # fourht pooling layer.
    with tf.name_scope('pool2'):
        h_pool4 = max_pool_2x2(h_conv4)      
        
    # lesser dropout 4
    with tf.name_scope('aux-dropout02'):
        c4_drop = tf.nn.dropout(h_pool4, convolution_keep_prob)
    
    # Fully connected layer 1 -- after 2 round of downsampling, our 28x28 image
    # is down to 1x1x256 feature maps -- maps this to 1024 features.
    with tf.name_scope('fc1'):
        W_fc1 = weight_variable([int(dimension_x / 100) * int(dimension_y / 100) * 128*4, 2048])
        print(np.shape(h_pool2))
        print(np.shape(h_pool3))
        print(np.shape(h_pool4))
        print(np.shape(W_fc1))
        b_fc1 = bias_variable([2048])

        h_pool2_flat = tf.reshape(c4_drop, [-1, int(dimension_x / 100) * int(dimension_y / 100) * 128*4])
    ...
    # Dropout - controls the complexity of the model, prevents co-adaptation of
    # features.
    with tf.name_scope('main-dropout2'):
        base_keep_prob = tf.placeholder(tf.float32)
        h_fc2_drop = tf.nn.dropout(h_fc2, base_keep_prob)    
    
    # Map the 1024 features to n classes, one for each digit
    with tf.name_scope('fc3'):
        W_fc3 = weight_variable([2048, classes])
