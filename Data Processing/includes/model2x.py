...
def cnnModel(training_data, channels, classes, dimension_x, dimension_y):
    ...
    with tf.name_scope('conv1'):
        W_conv1 = weight_variable([5, 5, channels, 64])
    ...
    with tf.name_scope('fc1'):
        W_fc1 = weight_variable([int(dimension_x / 10) * int(dimension_y /10) * 128, 1024])
        print(np.shape(h_pool2))
        print(np.shape(W_fc1))
        b_fc1 = bias_variable([1024])

        h_pool2_flat = tf.reshape(c2_drop, [-1, int(dimension_x / 10) * int(dimension_y / 10) * 128])
